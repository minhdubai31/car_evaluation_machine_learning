...
for i in  range(num_of_tests):
    ...
    # Save f1 score
    knn_f1_score.append(round(f1_score(y_test, y_pred, average="micro")*100,3))
    ...
    # Save f1 score
    bayes_f1_score.append(round(f1_score(y_test, y_pred, average="micro")*100,3))
    ...
    # Save f1 score
    forest_f1_score.append(round(f1_score(y_test, y_pred, average="micro")*100,3))    


# Export line chart displaying f1 scores of each model (to an image file)
export_line_chart(
    np.array(range(1, num_of_tests+1)), 
    [knn_f1_score, bayes_f1_score, forest_f1_score], 
...
export_feature_importances(forest_model.feature_importances_, X.columns, 'img/feature_importances/feature_importances.png')
...
print(f"Giá trị F1 trung bình Random Forest = {round(sum(forest_f1_score)/len(forest_f1_score), 3)}")
